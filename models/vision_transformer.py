...
class Attention(nn.Module):
    ...
    def forward(self, ref, obj):
        B, N, C = ref.shape
        kv = self.kv(ref).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1] # make torchscript happy (cannot use tensor as tuple)
        q = self.q(obj).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def forward(self, ref, obj):
        ref, obj = self.norm_ref(ref), self.norm_obj(obj)
        obj = obj + self.drop_path(self.attn(ref, obj))
        obj = obj + self.drop_path(self.mlp(self.norm2(obj)))
        return obj
